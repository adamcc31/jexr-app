 ...
 * 
 * Displays job details for public access (no authentication required).
 * Shows "Login to Apply" CTA for unauthenticated users.
 */
...
import { usePublicJob, useActiveJobs } from '@/hooks/useDiscovery';
import type { JobWithCompany } from '@/types/employer';
import {
    FiLayout, FiMapPin, FiUserCheck, FiClock, FiMonitor,
    FiBriefcase, FiDollarSign, FiBookmark, FiArrowUpRight, FiAlertCircle
} from 'react-icons/fi';
...
// Related job card component for public pages
            ...
            <div className="job-post job-type-three rounded shadow bg-white p-4">
                <div className="d-flex justify-content-between">
                    <div>
                        ...
                        <span className="h5 company text-dark d-block mt-2">
                            {job.company_name}
                        </span>
                    </div>
                </div>
                ...
                <div className="mt-2">
                    <Link href={`/jobs/${job.id}`} className="text-dark title h5">{job.title}</Link>
                ...
                <div className="container">
                    <div className="row g-4">
                        ...
                        <div className="col-lg-4 col-md-6">
                            <div className="card border-0 shadow rounded p-4 sticky-bar" style={{ marginTop: '-100px' }}>
                                ...
                                <div className="mt-4">
                                    ...
                                    {/* Apply CTA - Login required message for public users */}
                                    <div className="mt-4">
                                        <Link
                                            href={`/login?redirect=/candidate/jobs/${job.id}/apply`}
                                            className="btn btn-primary w-100"
                                        >
                                            <i className="mdi mdi-login me-2"></i>Login to Apply
                                        </Link>
                        ...
                        <div className="col-lg-8 col-md-6">
                            ...
                            {/* Apply CTA - Secondary placement */}
                            <div className="mt-4 pt-2 border-top">
                                <Link
                                    href={`/login?redirect=/candidate/jobs/${job.id}/apply`}
                                    className="btn btn-outline-primary"
                                >
                                    Login to Apply <i className="mdi mdi-send"></i>
                                </Link>
                            </div>
                        </div>
                    </div>
...
// Main page component
export default function PublicJobDetailPage({ params }: PageProps) {
    const resolvedParams = React.use(params);
    const jobId = parseInt(resolvedParams.id, 10);

    const { data: job, isLoading, isError, error } = usePublicJob(jobId);
    const { data: jobsData } = useActiveJobs(4);
    ...
    return (
        <>
            ...
            {!isLoading && !isError && !job && <JobNotFound />}
